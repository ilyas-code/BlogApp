...
function Dashboard() {
  ...
  const handleSave = useCallback(async () => {
    ...
    const date = new Date();
    // const data = { userName: usernamsede1, BlogText: savedData };
    const data = {
      likes: [""],
      reports: [""],
      UserName: "mohammed",
  ...
  return isAuthenticated ? (
    <>
      ...
      <Card className="text-left m-5 p-0">
        ...
        <div className="w-full flex gap-3 border justify-end px-4 py-3 ">
          <button
            type="submit"
            variant="success"
            disabled={posted}
            className={`${
              posted ? "bg-green-700" : ""
            } bg-[#3d7aed] text-white hover:bg-blue-700 font-semibold hover:text-white px-3 py-1 rounded-lg transition-all duration-150`}
            onClick={() => {
              handleSave();
              // SubmitHandle();
            }}
